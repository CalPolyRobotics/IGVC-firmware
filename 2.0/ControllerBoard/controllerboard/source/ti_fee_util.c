 ...
 uint16 TI_FeeInternal_FindNextVirtualSector(uint8 u8EEPIndex)
{
	...
	{		
		...
		{
			TI_Fee_u32ActCpyVS = 0x1U << (TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector-1U);
		}
		TI_Fee_GlobalVariables[u8EEPIndex].Fee_oStatus = TI_FEE_ERROR;		
	}	
 ...
 *  \note       TI FEE Internal API.
 *********************************************************************************************************************/
/*SAFETYMCUSW 27 D MR:8.10,8.11 <APPROVED> "Reason -  Cannot be static since it is used in format API."*/
		   ...
		   (TI_Fee_GlobalVariables[u8EEPIndex].Fee_au8VirtualSectorState[u16LoopIndex] == VsState_Invalid))
		{
			...
			if(TI_Fee_GlobalVariables[u8EEPIndex].Fee_Error == Error_Nil)
			{
				...
				for(u8Index = 0U ; u8Index<=(TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8VirtualSectorEnd-TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8VirtualSectorStart) ; u8Index++)
				{
					...
					(void)Fapi_setActiveFlashBank(Device_FlashDevice.Device_BankInfo[TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8Bank].Device_Core);
					/*SAFETYMCUSW 94 S MR:11.1,11.2,11.4 <APPROVED> "Reason -  Casting is required here."*/
					/*SAFETYMCUSW 95 S MR:11.1,11.4 <APPROVED> "Reason -  Casting is required here."*/
					...
					{						
						...
						(void)Fapi_enableEepromBankSectors(0xFFFFFFFFU, 0xFFFFFFFFU);
						#endif
					}										
...
void TI_FeeInternal_SanityCheck(uint16 BlockSize, uint8 u8EEPIndex)
{
	...
	while(bValid == TRUE)
	{
		...
		if(bValid == TRUE)
		{
			...
			bValid = TI_FeeInternal_BlankCheck(TI_Fee_GlobalVariables[u8EEPIndex].Fee_oFlashNextAddress,u32FlashNextAddress,(uint16)TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8Bank,u8EEPIndex);
...
TI_Fee_StatusType TI_FeeInternal_FeeManager(uint8 u8EEPIndex)
{
	...
	uint32 u32VirtualSectorHeaderAddress = 0U;
	boolean bDoNotPerformBlackCheckNow = FALSE;
	...
	if(FAPI_CHECK_FSM_READY_BUSY == Fapi_Status_FsmReady)
	{
		...
		if(TI_Fee_oStatusWord[u8EEPIndex].Fee_StatusWordType_ST.Copy == 1U)
		{
			...
			if(TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16BlockCopyIndex == 0xFFFFU)
			{
				...
				else if (TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16CopyVirtualSector != 0U)
				{
					...
					#if(TI_FEE_NUMBER_OF_EEPS==2U)
					if(((0U == u8EEPIndex) && ((TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector == TI_FEE_NUMBER_OF_VIRTUAL_SECTORS - TI_FEE_NUMBER_OF_VIRTUAL_SECTORS_EEP1) ||(TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector == 1U)))
						|| ((1U == u8EEPIndex) && ((TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector == TI_FEE_NUMBER_OF_VIRTUAL_SECTORS) ||
						(TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector == TI_FEE_NUMBER_OF_VIRTUAL_SECTORS  - TI_FEE_NUMBER_OF_VIRTUAL_SECTORS_EEP1+1U))))
					#elif(TI_FEE_NUMBER_OF_EEPS==1U)
					if((TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector == TI_FEE_NUMBER_OF_VIRTUAL_SECTORS) || 
		...
		#endif
		#if(TI_FEE_NUMBER_OF_EEPS==2U)
		else if(((0U == u8EEPIndex) && ((TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector == TI_FEE_NUMBER_OF_VIRTUAL_SECTORS - TI_FEE_NUMBER_OF_VIRTUAL_SECTORS_EEP1) ||(TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector == 1U)))
                || ((1U == u8EEPIndex) && ((TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector == TI_FEE_NUMBER_OF_VIRTUAL_SECTORS) ||
                   (TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16ActiveVirtualSector == TI_FEE_NUMBER_OF_VIRTUAL_SECTORS  - TI_FEE_NUMBER_OF_VIRTUAL_SECTORS_EEP1+1U))))
		#elif(TI_FEE_NUMBER_OF_EEPS==1U)
						...
						if((TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8InternalVirtualSectorEnd >= TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8InternalVirtualSectorStart))  
						{
							   ...
							   (0U == u8VSHeaderWriteCounter[u8EEPIndex]))
							{
															...
															)==Fapi_Status_Success)
								{
									...
									bDoNotPerformBlackCheckNow = TRUE;
									if(TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8InternalVirtualSectorEnd == TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8InternalVirtualSectorStart)
									{
							...
							if((TI_Fee_GlobalVariables[u8EEPIndex].Fee_Error == Error_Nil) && 
							   ...
							   (FALSE == bDoNotPerformBlackCheckNow))
							{
...
void TI_Fee_ErrorRecovery(TI_Fee_ErrorCodeType ErrorCode, uint8 u8VirtualSector)
{
	...
	switch(ErrorCode)
	{
		...
		case Error_NoFreeVS:			
			u32VirtualSectorStartAddress = TI_FeeInternal_GetVirtualSectorParameter(oSectorStart,
			                          /*SAFETYMCUSW 96 S MR:6.2,10.1,10.2,12.6 <APPROVED> "Macro comes from compiler files."*/
									  (uint16)TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8Bank,TRUE, u8EEPIndex);
			u32VirtualSectorEndAddress = u32VirtualSectorStartAddress+TI_FeeInternal_GetVirtualSectorParameter(
			            /*SAFETYMCUSW 96 S MR:6.2,10.1,10.2,12.6 <APPROVED> "Macro comes from compiler files."*/
						oSectorStart,(uint16)TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8Bank,FALSE, u8EEPIndex);
			/*SAFETYMCUSW 440 S MR:11.3 <APPROVED> "Reason - Casting is required here."*/
			if((Fapi_issueAsyncCommandWithAddress(Fapi_EraseSector,
