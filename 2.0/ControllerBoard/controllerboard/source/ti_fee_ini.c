...
#endif /* FEE_SW_MAJOR_VERSION */
#if (TI_FEE_SW_MINOR_VERSION != 16U)
	...
	{	
		   ...
		   (TI_Fee_GlobalVariables[u8EEPIndex].Fee_Error == Error_Nil))
		{
			...
			if(TI_Fee_GlobalVariables[u8EEPIndex].Fee_Error == Error_Nil)
			{
				...
				/* Enable Reading of all 1's as OK. Else Conqueror device may generate errors. */
				#ifdef _L2FMC
						...
						if((u32VirtualSectorStartAddress)!=0xFFFFFFFFU)
						{
							...
							{	
								...
								if(TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16CopyVirtualSector!=0U)
								{
									...
									TI_Fee_u32ActCpyVS = 0x1U << (TI_Fee_GlobalVariables[u8EEPIndex].Fee_u16CopyVirtualSector-1U);
									TI_Fee_u32ActCpyVS |= 0x1U << u16Index;
									TI_Fee_GlobalVariables[u8EEPIndex].Fee_oStatus=TI_FEE_ERROR;
								}
								else
				...
				{	
					...
					if(TI_Fee_GlobalVariables[u8EEPIndex].Fee_oStatus != TI_FEE_ERROR)
					{
						...
						/* Enable SECDED */
						Device_FlashDevice.Device_BankInfo[TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8Bank].Device_ControlRegister->EeCtrl1.EE_CTRL1_BITS.EE_EDACEN = 0xAU;
						/* Enable Single bit error correction */
						Device_FlashDevice.Device_BankInfo[TI_Fee_GlobalVariables[u8EEPIndex].Fee_u8Bank].Device_ControlRegister->EeCtrl1.EE_CTRL1_BITS.EE_EDACMODE = 0xAU;						
						#endif
