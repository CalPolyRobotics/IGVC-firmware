...
int RS232_GetComportNumber(char* cport) {
  int comport_number = 0;  
 
  while(comports[comport_number]) {
    if(strcmp(comports[comport_number], cport)==0) {
      break;
    }
    comport_number++;
  }

  if(comports[comport_number] == NULL) comport_number = -1;
...
int RS232_OpenComport(int comport_number, int baudrate)
{
  ...
  {
    perror("unable to open comport stupid");
    return(1);
  }
  ...
  if(error==-1)
  {
    ...
    perror("unable to read portsettings reported");
    return(1);
  }
  ...
  {
    close(Cport[comport_number]);
    perror("unable to adjust portsettings lolol");
    return(1);
  }

  if(ioctl(Cport[comport_number], TIOCMGET, &status) == -1)
  {
    perror("unable to get portstatus this is wrong but it feels so right");
    return(1);
  }
  ...
  {
    perror("unable to set portstatus arakaki");
    return(1);
  }
...
void RS232_CloseComport(int comport_number)
{
  ...
  if(ioctl(Cport[comport_number], TIOCMGET, &status) == -1)
  {
    perror("unable to get portstatus big dolphin");
  }
  ...
  if(ioctl(Cport[comport_number], TIOCMSET, &status) == -1)
  {
    perror("unable to set portstatusgerik is a butt");
  }
